(function () {
  ...
  function isActive(url) {
    return CURRENT === url || CURRENT === url.replace(/\/index\.html$/, "/");
  }
  ...
  function buildSection(key, pages, depth, storageKey) {
    ...
    if (key !== "__root__") {
      ...
      pages.forEach((page) => {
        const item = buildNavItem(page, depth);
      ...
      pages.forEach((page) => {
        section.appendChild(buildNavItem(page, depth));
      });
    }
      ...
      const filtered = query
        ? pages.filter(
            (p) =>
              ...
              (p.grade && String(p.grade).includes(query))
          )
        : pages;
  ...
  function initKeyboardNav() {
    ...
    container.addEventListener("keydown", (e) => {
      const items = [
        ...container.querySelectorAll(".nav-item, .nav-section-toggle"),
  ...
  function initIndexGrid() {
    ...
    for (const [key, pages] of Object.entries(NAV)) {
      ...
      const meta = CARD_META[key.toLowerCase()] || {
        icon: "📂",
